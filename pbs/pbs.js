...
// A days reduce availability; CQ, CI, Z do not.
...
// Bid period start/end dates — §23 A. 4.
// These are contractually fixed. Note that months do not align with calendar
// months — e.g. February runs Jan 31–Mar 1, March runs Mar 2–Mar 31.
...
? `<div style="font-size:0.85rem;color:var(--text-muted);margin-bottom:8px;word-break:break-all">${_session.user.email}</div> <button class="btn" onclick="signOut()">Sign Out</button>`
: `<input type="email" id="d-auth-email" placeholder="your@email.com" style="width:100%;margin-bottom:8px;padding:8px;border-radius:6px"> <button class="btn" onclick="sendMagicLink('d')">Send Magic Link</button> <div id="d-auth-msg" style="margin-top:8px;font-size:0.8rem;color:var(--text-muted)"></div>`;
const mContent = loggedIn
? `<div style="font-size:0.85rem;color:var(--text-muted);margin-bottom:8px;word-break:break-all">${_session.user.email}</div> <button class="btn" onclick="signOut()">Sign Out</button>`
: `<input type="email" id="m-auth-email" placeholder="your@email.com" style="width:100%;margin-bottom:8px;padding:8px;border-radius:6px"> <button class="btn" onclick="sendMagicLink('m')">Send Magic Link</button> <div id="m-auth-msg" style="margin-top:8px;font-size:0.8rem;color:var(--text-muted)"></div>`;
...
// Z displays as X — it is a personal reminder X-day, not a separate day type
...
// Available days = total days minus absence (A) days — §12 M. 2.
// CQ counts as on-call (R) for proration — it does not reduce availability.
// CI counts as on-call for proration — it does not reduce availability.
// Z counts as X for proration — it does not reduce availability.
const availCount = currDays.filter(d => d !== 'A').length;

// X used = X days + Z days — §12 M. 2.
// Z is a personal reminder X-day and counts as X for all rule purposes.
...
// extraX = pilot is in a category where reserves >= 20% of pilots — §12 M. 2. Note
const xReq = baseReq + (s.extraX ? 1 : 0);

if (xUsed !== xReq) alerts.push(xUsed < xReq ? `Need ${xReq - xUsed} more X` : `Too many X`);

// Count X-day blocks — §12 M. 8. d. (maxXblocks limit)
// Z counts as X for block-counting purposes, same as xUsed above.
// A days do not break an X block (absence days sit outside the schedule pattern).
...
// Cross-month work block validation — §12 M. 8. d. 2) (minWork)
// The previous month's days are prepended so a work block that spans the
// month boundary is evaluated as a single contiguous block. The offset
// tracks where the previous month ends and the current month begins, so
// illegalIndices can be mapped back to current-month day positions.
...
//
// Rules applied here:
//   - A block containing only CQ and/or CI days (no R or C) is always legal
//     regardless of length. CQ can be orphaned; CI always opens the month.
//     Only blocks containing at least one R or C day are subject to minWork.
//   - A block touching the last day of the current bid period is exempt from
//     minWork (end-of-month exception) — §12 M. 8. d. 2) Exception.
//   - A block exceeding maxWork is always flagged (default maxWork = 99,
//     effectively no limit in normal schedules).
...
// A block of only CQ days (orphaned training) or only CI days (carry-in
// at month start) is always legal regardless of length.
